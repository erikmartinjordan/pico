...
function createAutoZoomStream(sourceStream, region, options = {}) {
  ...
  const CURSOR_SLOW_SPEED          = 4.2;
  const CURSOR_FAST_SPEED          = 9.0;
  const CURSOR_JITTER_DEADZONE_PX  = 0.8 * scaleFactor;
  const CURSOR_FAST_SPEED_PX_S     = 2200 * scaleFactor;
  ...
  function draw(now = performance.now()) {
    ...
    lastFrameTime = now;
    const cursorDelta = Math.hypot(cursor.x - smoothedCursor.x, cursor.y - smoothedCursor.y);
    if (cursorDelta <= CURSOR_JITTER_DEADZONE_PX) {
      smoothedCursor.x = cursor.x;
      smoothedCursor.y = cursor.y;
    } else {
      const cursorSpeedRatio = clamp(cursorSpeedEMA / CURSOR_FAST_SPEED_PX_S, 0, 1);
      const adaptiveSmoothSpeed = CURSOR_SLOW_SPEED + (CURSOR_FAST_SPEED - CURSOR_SLOW_SPEED) * cursorSpeedRatio;
      smoothedCursor.x = expEase(smoothedCursor.x, cursor.x, adaptiveSmoothSpeed || CURSOR_SMOOTH_SPEED, dt);
      smoothedCursor.y = expEase(smoothedCursor.y, cursor.y, adaptiveSmoothSpeed || CURSOR_SMOOTH_SPEED, dt);
    }
